...
module.exports = class Microinvoice {
  ...
  generateTableRow(type, columns) {
    ...
    const _startY = this.storage.cursor.y;
		let pageJump = false;
    ...
    columns.forEach((column, index) => {
      ...
      // Increase y position in case of a line return
			// In case of a page jump, we need to reset the maxY to the current y position
			if (this.document.y < _startY) {
				if(pageJump === false) {
					_maxY = this.document.y;
				} else if(pageJump === true && this.document.y > _maxY) {
					_maxY = this.document.y;
				}
				pageJump = true;
			}
      if (this.document.y >= _maxY && pageJump === false) {
        _maxY = this.document.y;
      } 
    });
